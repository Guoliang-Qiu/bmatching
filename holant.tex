		  ...
		  \KwOut{A pair of assignments drawn from a coupling between $\mu^{\sigma}$ and $\mu^{\tau}$.}	
            ...
            }
            {
                ...
                \If{$\sigma_e \neq \tau_e$}
                {
                    ...
                    \Return $\left(\sigma_{S}\land \sigma',\tau_{S}\land \tau'\right)$
                }
                % \tcp{The disagreeing vertex remains the same}    
            }
            Sample $\sigma'\sim \mu^{\sigma}$\;
           \Return $(\sigma_{S}\land \sigma', \tau_{S}\land \sigma')$\;     
		%\Return{$(\sigma', \tau')$;}
	\end{algorithm}
\qtodo{The algorithm is wrong again?}
...
\begin{definition}[Linear Program Induced by the Coupling] \label{def:induced-LP}
    ...
    let $\+T = \+T_{\ell}(\Phi, \sigma_\bot, \tau_\bot, v_\bot)$ be the $\ell$-truncated coupling tree in \Cref{def:truncated-coupling-tree} and $0 \leq r^- \leq r^+\leq 1$ be two parameters satisfying
    \begin{align*}
...
\begin{lemma}[Building Cost of LP] \label{lem:building-cost-of-LP}
    For an instance $(\Phi, \sigma_\bot, \tau_\bot, v_\bot)$ satisfying~\Cref{cond-instancepair}, a non-negative integer $\ell \ge 0$ and two real numbers $0 \le r^- \le r^+\leq 1$,
...
\begin{proof}
    ...
    For a node $(\sigma, \tau, \seqS, v, L) \in \+L_{\!{good}}\cap \+V$, to compute $\frac{\mu(\tau)}{\mu(\sigma)}$, we have \qtodo{here should be changed when we add lambda}
    \qgl{
    \begin{align*}
        \frac{\mu(\tau)}{\mu(\sigma)} &= \frac{\sum_{x : E \to \set{0, 1} : x \in \tau} \prod_{u \in V} f_u\left(\abs{x ({E_u})}\right)}{\sum_{x : E \to \set{0, 1} : x \in \sigma} \prod_{u \in V} f_u\left(\abs{x ({E_u})}\right)}.
    \end{align*}}
    ...
    \begin{align*}
        \frac{\prod_{u \in V} f_u\left(\abs{x ({E_u})}\right)}{\prod_{u \in V} f_u\left(\abs{y ({E_u})}\right)} = \frac{f_v\left(\abs{\tau(E_v)}\right)}{f_v\left(\abs{\sigma({E_v})}\right)}
    \end{align*}
...
\begin{lemma}[Feasibility of LP] \label{lem:feasibility-of-LP}
    At least a feasible solution to the LP in~\Cref{def:induced-LP} exists.
\end{lemma}
\begin{proof}
...
\begin{theorem} \label{thm:ratio-bound-by-LP}
    Assume that all the constraints of the linear program defined as~\Cref{def:induced-LP} hold. Then it holds that
    \begin{align*}
...
Recall the definition of $R_{\Phi}(e)$ in \eqref{def-rphie}.


\begin{theorem} \label{thm:Holant-marginal-ratio-estimator}
    There exists a deterministic algorithm $\+A$ such that for each instance $(\Phi = (G, \vecf), \Delta, r)$ satisfying~\Cref{cond:Holant-condition}, each $e \in E(G)$, and each $\varepsilon \in (0, 1/4)$, within time $O\left(n \cdot \varepsilon^{-\poly(\Delta,r)}\right)$\zdtodo{check the running time}, $\+A$ outputs a number $\wh{R}$ such that
    $$
...
\begin{theorem} \label{thm:LP-ratio-estimator}
   There exists a deterministic algorithm $\+A_{\bot}$ such that for every instance $(\Phi = (G, \vecf), \sigma_\bot, \tau_\bot, v_\bot)$ satisfying~\Cref{cond-instancepair}, within time $O\left(n \cdot \varepsilon^{-\poly(\Delta(G),r_{\max}(\Phi))}\right)$ \zdtodo{check the running time}, $\+A_{\bot}$ outputs a number $\wh{R}$ such that
    $$
...
\begin{proof}[Proof of~\Cref{thm:Holant-marginal-ratio-estimator}]
    Given an instance $(\Phi = (G = (V, E), \vecf), \Delta, r)$ satisfying~\Cref{cond:Holant-condition} and an edge $e = \set{u, v} \in E$, if $f_u(1) = 0$ or $f_v(1) = 0$, we have $R_\Phi(e) = 0$ immediately. Thus, the algorithm 

    Otherwise, we have $f_u(1) > 0$ and $f_v(1) > 0$.
    One can verify that 
    \hktodo{the notation here}
...
\end{figure}
    ...
    Assume that $\Phi^{(1)}$ is the Holant instance after necessary removal. By definition, we have $\Phi^{(1)}$ and $\Phi^{(2)}$ both satisfy~\Cref{cond-instancepair} and $r_{\max}(\Phi^{(1)}) \le r$, $r_{\max}(\Phi^{(2)}) \le r$. Therefore, 
    \begin{align*}
