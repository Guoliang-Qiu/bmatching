...
\newcommand{\Par}{\!{Par}}
\newcommand{\Pick}{\!{Pick}}
\newcommand{\RunningTimeExponent}{\poly\left(\Delta(G), B(\Phi)^{-1}\right)}
...
$$ where $\sigma(E_v)$ is the restriction of $\sigma$ on the edges $E_v$.
...
Given any partial assignment $\sigma$ and vertex $v\in V$, recall that $E_v^\sigma$ is the set of unpinned edges incident to $v$ under $\sigma$. The following lemma in~\cite{CG24bMatching} establishes lower bounds of the marginal probability of $\mu$.
...
We say that a vertex $v \in V$ is \emph{weight-distinct under $(\sigma, \tau)$} if ${\!{Ham}\left(\sigma,{E_v}\right)}\neq{\!{Ham}\left(\tau,{E_v}\right)}$. Furthermore, a vertex $v$ is called \emph{1-distinct} under $(\sigma, \tau)$ if $\abs{{\!{Ham}\left(\sigma,{E_v}\right)}-{\!{Ham}\left(\tau,{E_v}\right)}}=1$. 
\hktodo{the definition of monotonicity is unclear}

Moreover, for a weight-distinct vertex $v$ and an edge $e \in E_v^\sigma$, we say that $e$ satisfies \emph{monotonicity} if $\mu_e^\sigma(1) \ge \mu_e^\tau(1)$ when $\Ham(\sigma, E_v) < \Ham(\tau, E_v)$ or $\mu_e^\sigma(1) \le \mu_e^\tau(1)$ otherwise.
We say that $(\sigma, \tau)$ is a pair of \emph{1-discrepancy partial assignments} if there is a unique weight-distinct vertex $v \in V$ and $v$ is 1-distinct.
...
\hktodo{polish the following paragraph}
Despite the challenges in constructing the random process $P^{\cp}$, we shall define a collection of quantities and verify some useful properties among them in $P^{\!{cp}}$. These quantities and properties provide the foundation for setting up the linear program.
...
\begin{proof}
    ...
    Combining \eqref{eq-def-bmin}, \eqref{eq:local-polynomial} with the assumption that $f_v(d) = 0$ for each $v\in V(G)$ and $d>\Delta$,
...
\end{figure}
    ...
    \begin{itemize}
    \item All of $\Delta\left(G^{(0)}\right), \Delta\left(G^{(1)}\right)$ and $\Delta\left(G^{(2)}\right)$ are no more than $\Delta(G)$;
    \item By $f_u(1)>0$, we have $f_u^{(1)}(0)>0$. Combined with  $\Phi = (G,\vecf)$ satisfies~\Cref{cond:Holant-condition}, one can verify that $\left(\Phi^{(1)},(e_v\leftarrow 1),(e_v\leftarrow 0),v\right)$ satisfies~\Cref{cond-instancepair}.
    ...
    % Similarly, we also have $r_{\max}(\Phi^{(2)}) \le r_{\max}(\Phi)$. 
        \item Recall the definition of $r_{\max}$ in~\eqref{eq-def-rmax} and $B$ in~\eqref{eq-def-bmin}. For the instance $\Phi^{(2)} = (G^{(2)}, \vecf)$, we have $r_{\max}(\Phi^{(2)}) = r_{\max}(\Phi)$ and $B(\Phi^{(2)}) = B(\Phi)$. For the instance $\Phi^{(1)} = \left(G^{(1)}, \vecf^{(1)}\right)$, we make the following claim:
        \begin{myclaim}
            It holds that $r_{\max}(\Phi^{(1)}) \le r_{\max}(\Phi)$ and $B(\Phi^{(1)}) \ge B(\Phi)$.
        \end{myclaim}
        \hktodo{write this in a lemma, and refer the the lemma here}
        For simplicity, we omit the proof here and provide it in~\Cref{sec:claim-in-ratio-estimator}.
    \end{itemize}
    ...
    \[
        \abs{V(G^{(1)})} \cdot \varepsilon_1^{-\poly\left(\Delta(G^{(1)}),1/B\left(\Phi^{(1)}\right)\right)} = \abs{V} \cdot \varepsilon^{-\RunningTimeExponent}
    \]
    where 
    \begin{align}\label{eq-rhat1}
        (1 - \varepsilon_1)R_{\Phi^{(1)}}(e_v) \leq \wh{R}_1 \leq (1 + \varepsilon_1)R_{\Phi^{(1)}}(e_v), 
    \end{align}
    Similarly, $\wh{R}_2$ can be obtained within time 
    \[
        \abs{V(G^{(2)})} \cdot \varepsilon_1^{-\poly\left(\Delta(G^{(2)}),1/B\left(\Phi^{(2)}\right)\right)} = \abs{V} \cdot \varepsilon^{-\RunningTimeExponent}
    \]
    where 
    \begin{align}\label{eq-rhat2}
        (1 - \varepsilon_2)R_{\Phi^{(2)}}(e_u) \leq \wh{R}_2 \leq (1 + \varepsilon_2)R_{\Phi^{(2)}}(e_u).
    \end{align}
    ...
    % \smallskip
    % \noindent\textbf{Proof of the claim.} To complete the whole proof, we show the claim. For $r_{\max}$, by~\eqref{eq-def-rmax} we have
    % \begin{align*}
    %     r_{\max}\left(\Phi^{(1)}\right) &= \max_{w \in V} \frac{f_w^{(1)}(1)}{f_w^{(1)}(0)} \\
    %     &= \max\set{\frac{f_u^{(1)}(1)}{f_u^{(1)}(0)}, \max_{w \in V \setminus \set{w}}\frac{f_w^{(1)}(1)}{f_w^{(1)}(0)}} \\
    %     (\mbox{by definition of $\vecf^{(1)}$}) \quad
    %     &= \max\set{\frac{f_u(2)}{f_u(1)}, \max_{w \in V \setminus \set{w}}\frac{f_w(1)}{f_w(0)}} \\
    %     (\mbox{by log-concavity of $\vecf$}) \quad
    %     &\le \max\set{\frac{f_u(1)}{f_u(0)}, \max_{w \in V \setminus \set{w}}\frac{f_w(1)}{f_w(0)}} = r_{\max}(\Phi).
    % \end{align*}
    
    % To show the inequality for $B$, it suffices to show that for every $w \in V$, we have
    % \begin{align} \label{eq:local-ratio-inequality}
    %     \frac{P_{f_w^{(1)}}(0)}{P_{f_w^{(1)}}(r_{\max}(\Phi^{(1)}))} \ge \frac{P_{f_w}(0)}{P_{f_w}(r_{\max}(\Phi))}.
    % \end{align}
    % By~\eqref{eq:local-polynomial}, $P_{f_w^{(1)}}(x)$ is increasing on $x \ge 0$. Recall that $0 \le r_{\max}(\Phi^{(1)}) \le r_{\max}(\Phi)$. To prove~\eqref{eq:local-ratio-inequality}, we only need to show
    % \begin{align} \label{eq:intermediate-local-ratio-inequality}
    %     \frac{P_{f_w^{(1)}}(0)}{P_{f_w^{(1)}}(r_{\max}(\Phi))} \ge \frac{P_{f_w}(0)}{P_{f_w}(r_{\max}(\Phi))}.
    % \end{align}
    % For every $w \in V \setminus \set{u}$, by definition of $\vecf^{(1)}$, $f_w^{(1)} = f_w$ and~\eqref{eq:intermediate-local-ratio-inequality} holds trivially. For $u$, suppose that $f_u = [f_u(0), \ldots, f_u(d)]$. With convention $f_u(d + 1) = 0$, we have
    % \begin{align*}
    %     &\symbolwidth \frac{P_{f_u^{(1)}}(0)}{P_{f_u^{(1)}}(r_{\max}(\Phi))} \\
    %     (\mbox{by~\eqref{eq:local-polynomial}}) \quad
    %     &= \frac{\sum_{i = 0}^{d} \binom{d}{i} f_u^{(1)}(i) 0^i}{\sum_{i = 0}^{d} \binom{d}{i} f_u^{(1)}(i) r_{\max}(\Phi)^i} \\
    %     &= \frac{f_u^{(1)}(0)}{\sum_{i = 0}^{d} \binom{d}{i} f_u^{(1)}(i) r_{\max}(\Phi)^i} \\
    %     \left(\mbox{by definition of $f_u^{(1)}$}\right) \quad
    %     &= \frac{f_u(1)}{\sum_{i = 0}^{d} \binom{d}{i} f_u(i + 1) r_{\max}(\Phi)^i} \\
    %     &= \frac{1}{\sum_{i = 0}^{d} \binom{d}{i} (f_u(i + 1) / f_u(1)) r_{\max}(\Phi)^i} \\
    %     (\mbox{by log-concavity of $f_u$}) \quad
    %     &\ge \frac{1}{\sum_{i = 0}^{d} \binom{d}{i} (f_u(i) / f_u(0)) r_{\max}(\Phi)^i} \\
    %     &= \frac{f_u(0)}{\sum_{i = 0}^{d} \binom{d}{i} f_u(i) r_{\max}(\Phi)^i} = \frac{P_{f_u}(0)}{P_{f_u}(r_{\max}(\Phi))}.
    % \end{align*}
\end{proof}
...
\begin{proof}
    Without loss of generality, assume that $\varepsilon < 1/4$, $\abs{E} = m$ and $E = \{e_1, \ldots, e_m\}$. For each $ i \in [m + 1]$, define $G_i \triangleq (V, E_i)$ where $E_i = E \setminus \set{e_1, \ldots, e_{i - 1}}$ 
    ...
    $\Phi_i$ satisfies~\Cref{cond:Holant-condition} for each $i \in [m+1]$. Recall the definition of $B$ in~\eqref{eq-def-bmin}, it holds that $B(\Phi_i) = B(\Phi)$.
    Thus, we have 
    \begin{equation}
    \begin{aligned}\label{eq-z-decompose}
        ...
        \left(\text{by $E_{m + 1} = \emptyset$}\right) \quad
    ...
    $$
        O\left(\abs{V(G_i)} (m/\varepsilon)^{\poly(\Delta(G_i), B(\Phi_i)^{-1})}\right) = O\left(\abs{V} \cdot (m/\varepsilon)^{\RunningTimeExponent}\right).
    $$
...
\begin{proof}
    ...
    $$
        O\left(\abs{V} \cdot (\abs{E} \cdot \varepsilon^{-1})^{\poly(\Delta(G), B(\Phi_{G, b})^{-1})}\right) = O\left(\abs{V} \cdot (\abs{E} \cdot \varepsilon^{-1})^{\poly(\Delta^b)}\right).
    $$
...
\section{Claim in the Proof of~\Cref{thm:Holant-marginal-ratio-estimator}} \label{sec:claim-in-ratio-estimator}

Recall the instances $\Phi = (G, \vecf)$ and $\Phi^{(1)} = \left(G^{(1)}, \vecf^{(1)}\right)$ in the proof of~\Cref{thm:Holant-marginal-ratio-estimator}. Now we prove the claim that $r_{\max}(\Phi^{(1)}) \le r_{\max}(\Phi)$ and $B(\Phi^{(1)}) \ge B(\Phi)$. For $r_{\max}$, by~\eqref{eq-def-rmax}, we have
\begin{align*}
    r_{\max}\left(\Phi^{(1)}\right) &= \max_{w \in V} \frac{f_w^{(1)}(1)}{f_w^{(1)}(0)} \\
    &= \max\set{\frac{f_u^{(1)}(1)}{f_u^{(1)}(0)}, \max_{w \in V \setminus \set{w}}\frac{f_w^{(1)}(1)}{f_w^{(1)}(0)}} \\
    (\mbox{by definition of $\vecf^{(1)}$}) \quad
...
To show the inequality for $B$, by~\eqref{eq-def-bmin}, it suffices to show that for every $w \in V$, we have
\begin{align} \label{eq:local-ratio-inequality}
    \frac{P_{f_w^{(1)}}(0)}{P_{f_w^{(1)}}(r_{\max}(\Phi^{(1)}))} \ge \frac{P_{f_w}(0)}{P_{f_w}(r_{\max}(\Phi))}.
\end{align}
By~\eqref{eq:local-polynomial}, $P_{f_w^{(1)}}(x)$ is increasing on $x \ge 0$. Recall that $0 \le r_{\max}(\Phi^{(1)}) \le r_{\max}(\Phi)$. To prove~\eqref{eq:local-ratio-inequality}, we only need to show
\begin{align} \label{eq:intermediate-local-ratio-inequality}
    \frac{P_{f_w^{(1)}}(0)}{P_{f_w^{(1)}}(r_{\max}(\Phi))} \ge \frac{P_{f_w}(0)}{P_{f_w}(r_{\max}(\Phi))}.
\end{align}
For every $w \in V \setminus \set{u}$, by the definition of $\vecf^{(1)}$, we have $f_w^{(1)} = f_w$ and~\eqref{eq:intermediate-local-ratio-inequality} holds trivially. For $u$, suppose that $f_u = [f_u(0), \ldots, f_u(d)]$. We make a convention that $f_u(i) = 0$ for every $i > d$. Since $f_u$ is log-concave, it holds that
\begin{align} \label{eq:log-concavity-inequality}
    \frac{f_u(i)}{f_u(0)} \ge \frac{f_u(i + \ell)}{f_u(\ell)}, \forall 0 \le \ell \le d, i \in \mathbb{N}.
\end{align}
Then, we have
\begin{align*}
    &\symbolwidth \frac{P_{f_u^{(1)}}(0)}{P_{f_u^{(1)}}(r_{\max}(\Phi))} \\
    ...
    &= \frac{f_u(1)}{\sum_{i = 0}^{d} \binom{d}{i} f_u(i + 1) r_{\max}(\Phi)^i} \\
    &= \frac{1}{\sum_{i = 0}^{d} \binom{d}{i} (f_u(i + 1) / f_u(1)) r_{\max}(\Phi)^i} \\
    (\mbox{by~\eqref{eq:log-concavity-inequality}}) \quad
    &\ge \frac{1}{\sum_{i = 0}^{d} \binom{d}{i} (f_u(i) / f_u(0)) r_{\max}(\Phi)^i} \\
    &= \frac{f_u(0)}{\sum_{i = 0}^{d} \binom{d}{i} f_u(i) r_{\max}(\Phi)^i} = \frac{P_{f_u}(0)}{P_{f_u}(r_{\max}(\Phi))}.
\end{align*}

Combining all of the above arguments, we conclude the claim.
