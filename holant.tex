...
One can verify that the following holds for the above quantities. The proof of the statement is deferred to~\Cref{sec:omitted Proofs}.
    ...
    \begin{align*}
        ...
        \end{align*}
        where the first inequality follows from the fact that the selection of the updated edges involves no randomness and \Cref{lem:marginal-bound}.
    \end{itemize}
     ...
     %    \end{align*}
    	% Similarly, we can show that $ p^{\tau'}_{\sigma',\tau',\seqS'} \geq  B_{\min} \cdot p^{\tau}_{\sigma,\tau,\seqS}$. 
...
The following lemma is the key ingredient to prove~\Cref{lem:LP-truncated-error}.
...
In this case, by \Cref{def:truncated-coupling-tree} we have $\+T_{0}$ is a tree with a unique node $(\sigma_\bot, \tau_\bot, \varnothing, v_\bot, 0)$.
In addition, by \Cref{def:induced-LP}, we have 
$\widehat{p}^{\sigma_\bot}_{\sigma_\bot,\tau_\bot, \varnothing} = 1$.
Therefore, 
 \begin{align*}
...
\begin{proof}[Proof of \Cref{lem:LP-truncated-error}]
...
\hktodo{use $(\sigma,\tau,\seqS)$ or use $(\sigma,\tau,\seqS,v,L)$}
