...
\begin{theorem} \label{thm:counting-b-matchings}
    ...
    \end{align*}
    in time $\poly\left((\abs{E} \cdot \varepsilon^{-1})^{C_{\ref{thm:counting-b-matchings}}}\right)$ where the coefficients of the polynomial and $C$ depends only on $\Delta$.
\end{theorem}
...
%     Let $\Delta$ be a positive integer and $r$ be a positive real number. Consider the Holant instance $\Phi=(G= (V, E),\vecf)$ satisfying
%     \begin{itemize}
%         \item The maximum degree of $G$ is bounded by $\Delta$.
%         \item $\vecf = \set{f_v}_{v \in V}$ is a family of binary symmetric log-concave signatures.
%         \item $f_v(1) \le r \cdot f_v(0)$ and $f_v(0) > 0$ for every $v \in V$.
%     \end{itemize}
% \end{condition}
\begin{condition} \label{cond:Holant-condition}
...
%     Given a positive integer $\Delta > 0$ and a real number $r > 0$, there is an FPTAS for the partition function of the Holant instance $\Phi$ satisfying~\Cref{cond:Holant-condition}. 
% \end{theorem}

\begin{theorem} \label{thm:counting-Holant}
    There is an algorithm such that given as input any $\varepsilon>0$ and any instance $\Phi = (G = (V, E), \vecf)$ satisfying~\Cref{cond:Holant-condition}, it outputs a number $\wh{Z}$ such that
    $$
        (1 - \varepsilon) Z_\Phi \le \wh{Z} \le (1 + \varepsilon) Z_{\Phi}
    $$
    \qgl{within time $\poly\left(\left(\abs{E} \cdot \varepsilon^{-1}\right)^{C_{\ref{thm:counting-Holant}}}\right)$ where coefficients of the polynomial and $C_{\ref{thm:counting-Holant}}$ depend only on $\Delta$ and $r$. Here $\Delta$ is the maximum degree of $G$ and $r=\max_v \frac{f_v(1)}{f_v(0)}$.} \qtodo{continue here later}
\end{theorem}
...
% Now we state some properties of Holant instances here. 
Let $\Phi = \left(G = (V, E), \vecf = \set{f_v}_{v \in V}\right)$ be a Holant instance satisfying~\Cref{cond:Holant-condition}. Recall that $\mu = \mu_{\Phi}$ is the corresponding Gibbs distribution. Define 
...
\end{align}
The following lemma in~\cite{CG24bMatching} bounds the marginal probability of $\mu$.
\begin{restatable}[Lemma 18 and Proposition 20 in~\cite{CG24bMatching}]{lemma}{MarginalBound} \label{lem:marginal-bound}
    Given any Holant instance $\Phi = \left(G , \vecf \right)$ satisfying~\Cref{cond:Holant-condition}, it holds that
    \begin{align} \label{eq:marginal-bound}
        \mu_{E_v^\sigma}^{\sigma}(\zero) \ge B_{\min}(\Phi),
    \end{align}
    for any feasible partial assignment $\sigma$ and vertex $v\in V$.
\end{restatable}
\begin{remark}
    \emph{\qgl{The original is state for the instance including no half edges, However, we can easily extend it to the half edge cases and we state the extended lemma here.}}
\end{remark}
...
For $e \in E$ and $c \in \set{0, 1}$, we use $Z^{e \gets c}=Z_{\Phi}^{e \gets c}$ to denote the partition function conditional on assigning $c$ to $e$, \IE,
\begin{align*}
    Z^{e \gets c}=Z_{\Phi}^{e \gets c} \defeq \sum_{\sigma \in \set{0, 1}^E :  \sigma(e) = c} \prod_{v \in V} f_v\left(\abs{\sigma ({E_v})}\right).
\end{align*}
One can verify that $R(e) = Z^{e \gets 1} / Z^{e \gets 0}$. 


The following lemma shows that we can efficiently check the feasibility of a partial assignment.

\begin{restatable}{lemma}{PartialFeasibility}\label{lem:partial-assignment-feasibility}
    Given any instance $\left(\Phi = (G = (V, E), \vecf = \set{f_v}_{v \in V}\right), \Delta, r)$ satisfying~\Cref{cond:Holant-condition} and any partial assignment $\sigma$, there is an algorithm determining whether $\sigma$ is feasible in time $O(\abs{\Lambda(\sigma)})$.
\end{restatable}
...
In the subsequent discussion, we consider instances satisfying the following condition.\qtodo{We may change all $\Phi = (G, \vecf)$ to $\Phi = (G, \vecf,\boldsymbol{\lambda})$}
...
\item $\+T$ is of degree at most $3\Delta$, of depth at most $\Delta \ell$ and of size at most $\left(3\Delta\right)^{\Delta \ell + 1}$.
Thus, for each node $(\sigma,\tau,\seqS,v,L)\in \+T$, we have $\abs{\Lambda(\sigma)} = \abs{\Lambda(\tau)} \leq \Delta \ell + 1$. \label{item:CT-property-size}
...
\begin{proof}
    ...
    Finally, one can immediately verify that the size of $\+T$ is no more than $\left(3\Delta\right)^{\Delta \ell + 1}$,  given that the depth is at most $\Delta \ell$ and the degree is at most $3\Delta$.
 
    \vspace{0.2cm}
    \underline{Proof of (3).} For each node $(\sigma, \tau, \seqS,v,L) \in \+L_{\!{good}}\cap \+V$, 
    \qtodo{here should be changed when we add lambda}
...
One can verify that the following proposition holds for the above quantities. The proof of the proposition is deferred to~\Cref{sec:omitted Proofs}.
    ...
    Recall the definitions of $B_{\min}$ and $\+D(\cdot)$ in \eqref{eq-def-bmin} and \Cref{def-notation-v-tct}. The next lemma provides the key property for bounding \hktodo{the coupling error}.
...
\begin{lemma}[Building Cost of LP] \label{lem:building-cost-of-LP}
    For any instance $(\Phi, \sigma_\bot, \tau_\bot, v_\bot)$ satisfying~\Cref{cond-instancepair}, any non-negative integer $\ell \ge 0$ and any two real numbers $0 \le r^- \le r^+\leq 1$ \zdtodo{the upper bound for $r^+$ cannot be $1$: the ratio can be $r_{\max}$.},
...
\end{lemma}
\begin{proof}
    First, we show that we can build the LP in time $\poly\left(\Delta^{\Delta \ell}\right)$. By Item~\eqref{item:CT-property-size} in~\Cref{prop:property-of-truncated-tree}, we can build the $\ell$-truncated coupling tree in time $\BuildTime$ and the feasibility of each $(\sigma, \tau, \seqS)$ can be checked in time $O(\abs{\seqS}) = O(\Delta \ell)$. Then the sets $\+V, \+L$ and $\+L_{\good}$ with $\+T$ can be obtained in time $O\left(\Delta \ell \cdot \BuildTime\right) = \BuildTime$.
    
    For every $(\sigma, \tau, \seqS) \in V(\+T)$, one can easily see that Constraints~\eqref{item-first-LP} to \eqref{item-third-LP} can be built in time $O\left(\abs{E_v^\sigma}\right) = O(\Delta)$. For any $(\sigma, \tau, \seqS) \in \+L_{\!{good}} \cap \+V$, by Item~\eqref{item:CT-property-ratio}, to build Constraint~\eqref{item-forth-LP}, it suffices to calculate the ratio $\frac{\mu(\tau)}{\mu(\sigma)} = \frac{f_v\left(\abs{\tau(E_v)}\right)}{f_v\left(\abs{\sigma(E_v)}\right)}$. Since $\tau(E_v)$ and $\sigma(E_v)$ can be determined in time $O(\abs{E_v}) = O(\Delta)$, we calculate the ratio in time $O(\Delta)$, meaning that the constraint can be built in time $O(\Delta)$. To build Constraint~\eqref{item-fifth-LP}, for each $(\sigma, \tau, \seqS) \in \+V \setminus \+L$, note that each possible sequence $\seqS'$ corresponds a permutation of edges in $E_v^\sigma$. Then we have $\abs{\+D} \le \abs{E_v^\sigma}! \le \Delta^\Delta$. For each $(\sigma, \tau, \seqS) \in V(\+T) \setminus \+V$, it is trivial that Constraint~\eqref{item-sixth-LP} can be built in constant time.

    Combining all the above, the LP can be built in time $O\left(\Delta^\Delta \BuildTime\right) = \BuildTime$. The bound for the time to check the feasibility of the linear program is a direct result of the building time.
    ...
    % 
    % To show the cost of building the linear program, it suffices to show the cost to verify whether a node $(\sigma, \tau, \seqS)$ is feasible or not, and compute the cost for building the constraints on $(\sigma, \tau, \seqS) \in \+L_{\!{good}}$.
    % 
    % For each $(\sigma, \tau, \seqS) \in V(\+T)$, 
    % by~\Cref{lem:partial-assignment-feasibility},
    % one can check the feasibility of $\sigma$ and $\tau$ in time $O(\abs{\Lambda(\sigma)}+\abs{\Lambda(\tau)})$.
    % In addition, by~\Cref{prop:property-of-truncated-tree} we have
    % $O(\abs{\Lambda(\sigma)}+\abs{\Lambda(\tau)})\leq O(\Delta \ell)$.
    % Thus, one can check whether $(\sigma, \tau, \seqS)$ is feasible in time $O(\Delta \ell)$.
    % 
    % Since each ratio can be calculated in time $O(\Delta)$, we can build the constraints on all good leaves in time $\poly\left(\Delta^{\Delta\ell}\right)$. \zdtodo{reconstruct this paragraph}
    %  
\end{proof}
...
\begin{theorem} \label{thm:Holant-marginal-ratio-estimator}
    There exists a deterministic algorithm $\+A$ such that for each instance $\Phi = (G = (V,E), \vecf)$ satisfying~\Cref{cond:Holant-condition}, each $e \in E$, and each $\varepsilon \in (0, 1/4)$, within time $O\left(\abs{V}\cdot \varepsilon^{-\poly(\Delta(G),r_{\max}(\Phi))}\right)$\zdtodo{check the running time}, $\+A$ outputs a number $\wh{R}$ such that
    $$
...
\begin{theorem} \label{thm:LP-ratio-estimator}
   There exists a deterministic algorithm $\+A_{\bot}$ such that for every instance $(\Phi = (G, \vecf), \sigma_\bot, \tau_\bot, v_\bot)$ satisfying~\Cref{cond-instancepair}, within time $O\left(n \cdot \varepsilon^{-\poly(\Delta(G),r_{\max}(\Phi))}\right)$ \zdtodo{check the running time: the exponent is not polynomial}, $\+A_{\bot}$ outputs a number $\wh{R}$ such that
    $$
...
\begin{proof}
    For simplicity, let $r = r_{\max}(\Phi)$ and $\Delta = \Delta(G)$ and 
    $$
    ...
    \begin{align*}
        C_1(\Delta, r) &= \Delta \log{\Delta} \left(1 + \frac{3 + \log{r}}{-\log\left(1 - (1 + r^2)^{-2\Delta}\right)}\right) \le \left(1 + (3 + \log{r})(1 + r^2)^{2\Delta}\right) \Delta \log{\Delta},\\
        C_2(\Delta, r) &= \frac{\Delta \log \Delta}{-\log\left(1 - (1 + r^2)^{-2\Delta}\right)} \le (1 + r^2)^{2\Delta} \Delta \log{\Delta}
    \end{align*}
    where the inequalities follow from the inequality $-\log{\left(1 - \frac{1}{x}\right)} \le \frac{1}{x}$ for $x > 1$.
    \zdnew{Then by~\Cref{lem:building-cost-of-LP}, the running time of the marginal estimator is $\poly\left(\varepsilon^{-\exp(\Delta \log{r})}\right)$.}
\end{proof}
...
\end{figure}
    ...
    \[O\left(\abs{V} \cdot \varepsilon_1^{-\poly(\Delta(G^{(1)}),r_{\max}(\Phi^{(1)}))}\right) = O\left(\abs{V} \cdot \varepsilon^{-\poly(\Delta(G),r_{\max}(\Phi))}\right),\]
    where 
    ...
    \[O\left(\abs{V} \cdot \varepsilon_2^{-\poly(\Delta(G^{(2)}),r_{\max}(\Phi^{(2)}))}\right) = O\left(\abs{V} \cdot \varepsilon^{-\poly(\Delta(G),r_{\max}(\Phi))}\right),\]
    where 
   ...
   (1 - \varepsilon')R_{\Phi^{(2)}}(e_u) \leq \wh{R}_2 \leq (1 + \varepsilon')R_{\Phi^{(2)}}(e_u).
    ...
    At last, we consider the time cost for calculating $\wh{R}$.
